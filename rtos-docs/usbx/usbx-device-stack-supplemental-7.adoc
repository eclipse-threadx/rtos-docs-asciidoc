...
= Chapter 7: USBX Device Class Implementation Considerations
...
The implementation then handles USBX device stack commands to:

* initialize and uninitialize the class/function;
* query whether the class/function is supported;
* activate and deactivate the class/function;
* process received class-specific requests;
* handle interface alternate setting changes.
...
| (UX_SLAVE_CLASS*)
| (VOID*)param
| Initialize class/function module instance

| UX_SLAVE_CLASS_COMMAND_QUERY
| (UX_SLAVE_INTERFACE*) | ULONG
| (UX_SLAVE_CLASS*)
| not used
| Query if class/subclass/protocol is supported
...
| (UX_SLAVE_CLASS*)
| not used
| Activate local class/function instance
...
| UX_SLAVE_CLASS_COMMAND_REQUEST (optional)
| not used
| not used
| (UX_SLAVE_CLASS*)
| not used
| Handle class/vendor requests from host (if needed)

| UX_SLAVE_CLASS_COMMAND_CHANGE (optional)
| (UX_SLAVE_INTERFACE*)
| not used
| (UX_SLAVE_CLASS*)
| not used
| Interface alternate setting change notification (if needed)

| UX_SLAVE_CLASS_COMMAND_UNINITIALIZE (optional)
| not used
| not used
| (UX_SLAVE_CLASS*)
| not used
| Uninitialize class/function module instance
|===
