...
= Chapter 6: USBX device controller driver (DCD) Porting Considerations
...
* <<ux_dcd_xxxxx_uninitialize,ux_dcd_xxxxx_uninitialize>>

[[_ux_dcd_xxxxx_initialize]]
=== Initialize API (_ux_dcd_xxxxx_initialize)
...
[[ux_dcd_xxxxx_uninitialize]]
=== Uninitialize API (_ux_dcd_xxxxx_uninitialize) (optional)
...
| UX_SLAVE_TRANSFER*
| Issue a transfer/run transfer state machine

| UX_DCD_TRANSFER_ABORT
| UX_SLAVE_TRANSFER*
| Abort on going transfer
...
| ULONG
| Return status of endpoint (index/address)
...
| UX_DCD_CHANGE_STATE
| ULONG (UX_DEVICE_REMOTE_WAKEUP)
| Change USB state (to remote wakeup)

| UX_DCD_ISR_PENDING/UX_DCD_TASKS_RUN
| -
| For standalone, run background task state machine
|===
