...
NOTE: Several flags are set and cleared internally by GUIX. Modifying them directly via the status API leaves GUIX in an unspecified state, and GUIX makes no guarantees about its subsequent behavior. Flags documented as "managed internally" are described here for reference purposes only.
