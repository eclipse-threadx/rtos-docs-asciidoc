...
image::guix/image6.jpg[Screenshot of the drop list.]
...
image::guix/image7.jpg[Screenshot of the Studio properties view.]
...
IMPORTANT: _This service creates the GUIX event queue, system thread, timer and protection mutex, all of which are ThreadX objects, so the earliest point it may be called from is_ *_tx_application_define_*. _Calling it before_ *_tx_kernel_enter_* _returns_ *GX_SUCCESS* _and leaves GUIX unusable, because_ *_tx_kernel_enter_* _resets the kernel lists those objects were linked onto._
