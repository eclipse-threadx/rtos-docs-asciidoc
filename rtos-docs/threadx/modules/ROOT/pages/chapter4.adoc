...
IMPORTANT: _It is important to ensure application code does not write outside the allocated memory block. If this happens, the behavior is undefined. Likewise, if the destination is not large enough, the behavior is undefined. That said, a typical consequence is memory corruption of an adjacent (usually subsequent) memory area, which is often fatal!_
...
WARNING: _This service should not be used to enable interrupts during initialization. If it is, ThreadX makes no guarantees about the resulting system state._
