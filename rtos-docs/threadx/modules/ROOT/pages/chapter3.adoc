...
image::user-guide/types-program-execution.png[Types of Program Execution]
...
image::user-guide/memory-area-example.png[Memory Area Example]
...
Interrupts are left disabled during the entire initialization process. If the application somehow enables interrupts, ThreadX makes no guarantees about the resulting system state. Figure 4 shows the entire
...
image::user-guide/initialization-process.png[Initialization Process]
...
image::user-guide/thread-state-transition.png[Thread State Transition]
...
image::user-guide/typical-thread-stack.png[Typical Thread Stack]
...
image::user-guide/stack-preset.png[Stack Preset to 0xEFEF*]
...
A stack overflow results in undefined behavior. That said, a typical consequence is that thread execution corrupts the memory adjacent to (usually before) its stack area, which most often shows up as an unnatural change in the program counter. This is often called "jumping into the weeds." Of course, the only way to prevent this is to ensure all thread stacks are large enough.
...
image::user-guide/example-suspended-threads.png[Example of Suspended Threads]
...
It is important to ensure that the user of an allocated memory block does not write outside its boundaries. If this happens, the behavior is undefined. That said, a typical consequence is memory corruption of an adjacent (usually subsequent) memory area, which is often fatal to the application.
...
It is important to ensure that the user of allocated memory does not write outside its boundaries. If this happens, the behavior is undefined. That said, a typical consequence is memory corruption of an adjacent (usually subsequent) memory area, which is often catastrophic for program execution.
