...
A driver that protects itself with a lock of its own rather than with *TX_DISABLE* and *TX_RESTORE* must not hold that lock across a ThreadX SMP service call made from an ISR. See "Application Locks and the ThreadX SMP Protection" in xref:threadx:ROOT:threadx-smp/chapter3.adoc[Chapter 3 - Functional Components of ThreadX SMP].
