...
Interrupts are left disabled during the entire initialization process. If the application somehow enables interrupts, ThreadX SMP makes no guarantees about the resulting system state. Figure 3 on page 52 shows the entire initialization process, from system reset through application-specific initialization.
...
A stack overflow results in undefined behavior. That said, a typical consequence is that thread execution corrupts the memory adjacent to (usually before) its stack area, which most often shows up as an un-natural change in the program counter. This is often called "jumping into the weeds." Of course, the only way to prevent this is to ensure all thread stacks are large enough.
...
It is important to ensure that the user of an allocated memory block does not write outside its boundaries. If this happens, the behavior is undefined. That said, a typical consequence is memory corruption of an adjacent (usually subsequent) memory area, which is often fatal!
...
It is important to ensure that the user of allocated memory does not write outside its boundaries. If this happens, the behavior is undefined. That said, a typical consequence is memory corruption of an adjacent (usually subsequent) memory area, which is often fatal!
...
=== Application Locks and the ThreadX SMP Protection

ThreadX SMP serialises its internal data structures with a single global protection lock, taken by *TX_DISABLE* and released by *TX_RESTORE*. Every service in the list above takes that lock.

A core may hold the protection with interrupts enabled. This happens wherever a service defers preemption across a long operation, such as resuming the threads suspended on an object being deleted, or the interval between the suspension request in _tx_thread_sleep_ and the context switch that carries it out. A core in one of those windows owns the protection and can still take an interrupt.

That fixes the lock order. The ThreadX SMP protection is the outer lock, and any application lock an ISR can take is acquired inside it. An ISR must therefore never call a ThreadX SMP service while holding such a lock: one core would spin for the protection while holding the application lock, and the core owning the protection would spin for the application lock in its own interrupt handler. Neither ever completes.

Two ways to respect the order:

* Use *TX_DISABLE* and *TX_RESTORE* as the application's inter-core lock rather than a private spinlock. They already provide inter-core protection, and they nest on the owning core, so a service call from inside the critical section is safe.
* Release the private lock before calling the service. Copy out whatever the call needs, unlock, then call.
