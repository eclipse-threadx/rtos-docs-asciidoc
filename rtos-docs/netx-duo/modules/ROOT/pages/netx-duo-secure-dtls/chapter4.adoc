...
NOTE: Disabling client authentication on an active DTLS Server instance leaves the server in an unspecified state, and NetX Duo Secure makes no guarantees about the behavior of sessions already in progress. The nx_secure_dtls_server_stop service should be called before changing server state.
