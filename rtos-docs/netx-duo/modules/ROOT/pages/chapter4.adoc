...
Under the "Allowed From" heading, "Initialization" means from *_tx_application_define_* onward. It does not extend to code that runs before *_tx_kernel_enter_*, such as a C++ static constructor or preliminary processing in _main_. See "Application Interface Calls" in xref:chapter3.adoc[Chapter 3 - Functional Components of NetX Duo].
...
IMPORTANT: _This service creates the IP instance's periodic timer in the active state, so it must not be called before_ *_tx_kernel_enter_*. _Doing so faults rather than returning an error, because the list of active timers does not exist yet._
...
WARNING: _Unless an error is returned, the application should not release the packet after this call. Doing so results in undefined behavior, because the network driver releases the packet after transmission and the application would release it a second time_.
...
CAUTION: _The destination buffer must be large enough to hold the packet's contents. If it is not, the behavior is undefined. That said, a typical consequence is memory corruption of the area following the buffer_.
...
____
[!NOTE] +
_The application must prevent releasing a packet more than once, because releasing a packet twice results in undefined behavior_.
____
...
WARNING: _Unless an error is returned, the application should not release the packet after this call. Doing so results in undefined behavior, because the network driver also releases the packet after transmission and the application would release it a second time_.
