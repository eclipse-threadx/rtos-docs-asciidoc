...
WARNING: Unless an error is returned, the application should not release the allocated RTP packet by calling *_nx_packet_release()_* after calling the function *_nx_packet_data_append()_* or *_nx_rtp_sender_session_packet_send()_*. Doing so results in undefined behavior, because the network driver also releases the packet after transmission and the application would release it a second time.
