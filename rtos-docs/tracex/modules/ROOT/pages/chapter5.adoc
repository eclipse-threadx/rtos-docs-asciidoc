...
Under the "Allowed From" heading in the service descriptions below, "Initialization" means from *_tx_application_define_* onward. It does not extend to code that runs before *_tx_kernel_enter_*, such as a C++ static constructor or preliminary processing in _main_. See "Calling ThreadX Services During Initialization" in xref:threadx:ROOT:chapter3.adoc[Chapter 3 of the ThreadX User Guide].
...
IMPORTANT: *_tx_application_define_* is the earliest point from which this service may usefully be called. It registers the ThreadX objects that exist at the time of the call, and before *_tx_kernel_enter_* there are none. Depending on how the library is built, *_tx_kernel_enter_* also clears the trace buffer pointers this service sets up, in which case the earlier call returns *TX_SUCCESS* and leaves tracing disabled.
