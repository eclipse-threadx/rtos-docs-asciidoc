...
NOTE: _This service assigns only LevelX's own global state and has no ThreadX dependency, so unlike most services listed as allowed from Initialization it may also be called before_ *_tx_kernel_enter_*. _In a standalone build --_ *LX_STANDALONE_ENABLE* _defined -- there is no ThreadX at all. The LevelX service that does create a ThreadX object is_ *_lx_nand_flash_open_*, _which is not allowed from Initialization._
