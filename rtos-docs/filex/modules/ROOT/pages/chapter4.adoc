...
IMPORTANT: _*_tx_application_define_* is the earliest point from which this service may be called. Unless the FileX library is built with_ *FX_NO_TIMER* _defined, it creates the FileX system timer in the active state, which places the timer on a kernel list that does not exist until_ *_tx_kernel_enter_* _sets it up. Calling this service before then faults rather than returning an error._
